...
export async function getRandomImages(number = 1) {
    ...
    const promises = Array(number).fill().map(() => fetchImage(apiUrl))
...
export function formatTime(time, { maxThreeUnits = true, showCent = true } = {}) {
    const hours = parseInt(time / (60 * 60 * 1000))
    const minutes = parseInt((time / (60 * 1000)) % 60)
    const seconds = parseInt((time / 1000) % 60)
    const centiseconds = parseInt((time % 1000 / 10))
